...
\cw{TODO: Mention information theoretic bottlenemck 2511.04681 and refs therein; variational lower bound on mutual information}
\[
\mathcal{L}_{\text{NPE}} 
\;\geq\;
\mathbb{E}_{p(\bx)}
...
Using the identity \cw{Variational lower bound}
\begin{equation}
\mathbb{E}_{p(\bx)}
\mathcal{H}(p(\btheta \mid T_\phi(\bx)))
...
\subsection{Conditional Generative Models for Simulation-Based Inference}

Any generative model that produces parameter samples $\btheta$ can be turned into a simulation-based inference method by conditioning it on data $\bx$ and training it on parameter--data pairs drawn from the forward model, $p(\bx \mid \btheta)\, p(\btheta)$. This observation provides a unifying perspective on simulation-based inference and explains why recent advances in generative modeling can often be incorporated into SBI with minimal conceptual effort.

\paragraph{Unconditional generative models}

In order to see this, we begin with a generic formulation of generative modeling. A generative model is defined by a \emph{map} that transforms samples from a simple tractable base distribution into samples of interest,
%
\begin{equation}
    \btheta = G_\phi(\bepsilon), \qquad \bepsilon \sim p(\bepsilon)\;.
\end{equation}
%
Here, the \emph{generator} $G_\phi$ induces a model distribution over $\btheta$, which can be written in pushforward notation as,
%
\begin{equation}
q_\phi(\btheta) = (G_\phi)_\#\, p(\bepsilon)\;.
\end{equation}
%
Note that samples $\btheta \sim q_\phi(\btheta)$ are obtained by pushing samples from $p(\bepsilon)$ through the generator, whereas evaluating the density $q_\phi(\btheta)$ requires inversion of the map and access to the corresponding log-determinant.

Generally, the parameters $\phi$ of the generator are learned by minimizing a suitable loss, averaged over target samples $\btheta \sim p(\btheta)$,
%
\begin{equation}
    \phi^\star
    =
    \arg\min_\phi
    \;
    \mathbb{E}_{\btheta \sim p(\btheta)}
    \big[
    \mathcal{L}\big(\btheta, G_\phi\big)
    \big]
    \quad
    \Rightarrow
    \quad
    q_{\phi^\ast}(\btheta) \approx p(\btheta).
\end{equation}
%
For an appropriately chosen loss function, the induced model distribution provides then an accurate approximation to the target distribution.

\paragraph{Conditional generative models for SBI}

\emph{Conditioning} the generator on an auxiliary variable $\bx$ means allowing the generative map---and hence the distribution of generated parameter samples---to depend explicitly on the data.  This yields a conditional generative model,
%
\begin{equation}
\btheta = G_\phi(\bx,\bepsilon),
\qquad
\bepsilon \sim p(\bepsilon)\;.
\end{equation}
%
For a fixed observation $\bx$, the generator then defines a conditional distribution over the parameters, which in pushforward notation can be written as
%
\begin{equation}
q_\phi(\btheta \mid \bx)
=
(G_\phi(\bx,\cdot))_\#\, p(\bepsilon)\;.
\end{equation}
%
Sampling and evaluation from this conditional distribution generally proceeds as we discussed above.

In simulation-based inference, training data are drawn from the forward model $p(\bx \mid \btheta)p(\btheta)$. Importantly, the conditional generator is trained by \emph{minimizing the same loss} as in the unconditional case, now averaged over this joint distribution,
%
\begin{equation}
\phi^\star
=
\arg\min_\phi
\;
\mathbb{E}_{(\btheta,\bx)\sim p(\btheta)p(\bx\mid\btheta)}
\big[
\mathcal{L}\big(\btheta, G_\phi(\bx,\cdot)\big)
\big]
\quad
\Rightarrow
\quad
q_{\phi^\ast}(\btheta\mid \bx) \approx p(\btheta\mid \bx).
\end{equation}
%
The learned induced distribution approximates then by construction the true posterior $p(\btheta \mid \bx)$.

\medskip

Different choices of the loss $\mathcal{L}$ recover a wide range of inference methods, including likelihood-based density estimation, score-based and diffusion models, flow-matching approaches, as well as adversarial or contrastive objectives. In this sense, conditional generative modeling provides a unifying abstraction for modern simulation-based inference methods.
\cw{TODO: Link to below and previous sections}
...
\paragraph{Connecting base and target distributions through ODEs}
Like for CNFs, the goal is to transform parameters $\btheta_0$ drawn from a simple and tractable base distribution $p_0(\btheta)$ into parameters $\btheta \equiv \btheta_1$ that are effecitvely drawn from the model posterior $p(\btheta \mid \bx)$.  The transformation is modeled as a solution to the ODE
%
\begin{equation}
    \frac{d}{dt}\btheta_t = \mathbf{v}_\phi(\btheta_t, \bx, t) \quad \text{with} \quad \btheta_0 \sim p_0(\btheta_0),
\end{equation}
%
with time parameter $t\in [0, 1]$, and where $\mathbf{v}_\phi$ is a learnable velocity field in parameter space.  Since we are interested in modeling a data conditioned distribution, this velocity field has to be conditioned on $\bx$.

The above ODE gives rise to a family of distributions of the intermediate parameters $\btheta_t$, which we call $q_{\phi, t}(\btheta_t \mid \bx)$. Its evolution is given by the standard \emph{continuity equation}, 
%
\begin{equation}
    \frac{\partial}{\partial t} q_{\phi, t}(\btheta \mid \mathbf{x}) = -\nabla_{\btheta} \cdot \big(q_{\phi, t}(\btheta \mid \bx) \mathbf{v}_\phi(\btheta, \bx, t)\big)\;,
    \label{eqn:flow_matching_theta}
\end{equation}
%
which implies that the derivative of the density field is given by the negative divergence of the velocity field,

\medskip

As for CNFs, the training goal is that the endpoint at $t=1$ approximates the full posterior:
\[
q_\phi(\btheta \mid \bx) \equiv q_{\phi,1}(\btheta_{1} \mid \bx, 1) \approx p(\btheta \mid \bx).
\]
However, in contrast to CNFs which are directly matching the learned distribution $q_\phi(\btheta\mid \bx)$ at $t=1$ against the target $p(\btheta \mid \bx)$ through maximum likelihood objectives, flow matching train the velocity field for all values of $t$. 

\paragraph{Training objective} While CNF leaves the path connecting to base distribution at $t=0$ and the target distribution at $t=1$ essentially unconstrained, flow matching requires to define a connecting path for all values of $\btheta_t$, and the corresponding velocity field consistent with Eq.~\eqref{eqn:flow_matching_theta}, upfront.

Although there is in principle high flexibility in defining the transport map from base samples $\btheta_0$ to target samples $\btheta_1$, the implied velocity field has to remain tractable. A typical starting point is to define the path through convolution with a distortion function,
\[
...
The distortion function $p_t(\btheta_t \mid \btheta_1)$ takes samples from the target distribution and redistributes them at some earlier time $t<1$. A common ansatz is to define a distortion function 
\[
p_t(\btheta_t \mid \btheta_1 ) = \mathcal{N}(\btheta_t \mid \alpha_t \btheta_1, \sigma_t^2 \mathbf{I})
\]
which rescales $\btheta_1$ depending on the scheduling function \cw{???} with boundary conditions $\alpha_t$ and $\sigma_t$, where $\alpha_0 = 1$, $\alpha_1=0$, $\sigma_1 = 0$, $\sigma_1 = 1$.

After defining the path, the remaining challenge is to identify the velocity field that would lead to this probability distribution path. Although this is generally challenging, for the above construction, 
it can be shown 
\citep{lipman_flow_2023, wildberger_flow_2023}
that tractability of the vector field that generates the conditional distribution $p_t(\btheta_t \mid \btheta_1, \bx)$ is enough to estimate the vector field for unconditional distribution. In fact, through application of the continuity equation one can show that
\[
\mathbf{u}(\btheta_t, \bx) = \mathbb{E}_{\btheta_1 \sim p(\btheta_1 \mid \btheta_t, \bx)} [\mathbf{u}(\btheta_t \mid \btheta_1)]\;,
\]
where $\mathbf{u}(\btheta_t, \bx)$ denotes the velocity field of the \emph{target} path, that we aim to approximate through the neural velocity field $\mathbf{v}_\phi$, and $\mathbf{u}(\btheta_t \mid \btheta_1)$ is the velocity field of the path defined by the distortion function $p_t(\btheta_t \mid \btheta_1)$.
One can show that the conditional velocity field is given can be estimated as
$$
\mathbf{u}(\btheta_t \mid \btheta_1) = \mathbb{E}[\alpha'_t \btheta_1 + \sigma'_t \boldsymbol{\varepsilon} \mid \btheta_t ]\;.
$$
...
t\sim \mathcal{U}(0, 1),
\btheta \sim p(\bx \mid \btheta) p(\btheta),
\btheta_t \sim p(\btheta_t \mid \btheta_1 = \btheta)
}
\|\mathbf{v}_\phi(\btheta_t, \bx, t) - \mathbf{u}(\btheta_t \mid \btheta_1 = \btheta)
\|^2.
\]
...
Stochasticity can be introduced via the SDE
\begin{equation}
d\btheta_t = \mathbf{f}_{\phi}(\btheta_t, \mathbf{x}, t)dt + g(t)d\mathbf{W}
\quad
\text{with}
\quad
\btheta_0 \sim p_0(\btheta)
\end{equation}

This induces a probability distribution that evolves like the Fokker-Planck equation,

\begin{equation}
    \frac{\partial}{\partial t} q_{\phi, t}(\btheta \mid \mathbf{x}) = -\nabla_{\btheta} \cdot \big(q_{\phi, t}(\btheta \mid \bx) \mathbf{f}_\phi(\btheta, \bx, t)\big)
    + \frac12 g(t)^2\, \nabla_{\btheta}^2\, q_{\phi, t}(\btheta \mid \bx)
    \;,
    \label{eqn:flow_matching_theta}
\end{equation}

Although individual paths are fundamentally different to ODE, for each SDE one can find an equavlent ODE with the velocity field
%
\begin{equation}
    \mathbf{v}_\text{eff}(\btheta, \bx, t) = \mathbf{f}_\phi(\btheta, \bx, t)
    - \frac12 g(t)^2 \nabla_{\btheta} \log q_{\phi, t}(\btheta \mid \bx)
\end{equation}
%
Similar solution space, but different learning dynamics.
...
\subsubsection{Diffusion Denoising Probabilistc Models}
